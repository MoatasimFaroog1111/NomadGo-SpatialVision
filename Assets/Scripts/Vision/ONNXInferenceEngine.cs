...
namespace NomadGo.Vision
{
    public class ONNXInferenceEngine : MonoBehaviour
    {
        ...
        private float confidenceThreshold = 0.05f;
        ...
        public void Initialize(AppShell.ModelConfig config)
        {
            ...
            confidenceThreshold = Mathf.Clamp(config.confidence_threshold <= 0 ? 0.05f : config.confidence_threshold, 0.01f, 0.90f);
            if (confidenceThreshold > 0.10f) confidenceThreshold = 0.05f;
            nmsThreshold = Mathf.Clamp(config.nms_threshold <= 0 ? 0.45f : config.nms_threshold, 0.10f, 0.95f);
            maxDetections = Mathf.Clamp(config.max_detections <= 0 ? 50 : config.max_detections, 1, 200);
        ...
        public void ReloadModel(string onnxPath, string newLabelsPath)
        {
            ...
            if (!string.IsNullOrEmpty(newLabelsPath) && File.Exists(newLabelsPath))
                labels = File.ReadAllText(newLabelsPath).Split(new[] { '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries);
            StartCoroutine(LoadModelAsync());
        }

        public List<DetectionResult> RunInference(Texture2D frame)
        {
            if (frame == null || !isLoaded) return new List<DetectionResult>();
        ...
        {
            string res = string.IsNullOrEmpty(labelsPath) ? "labels" : labelsPath.Replace(".txt", "").Replace("Models/", "").Replace("Resources/", "");
            ...
            if (asset != null)
                ...
                labels = new[] { "person","bicycle","car","motorcycle","airplane","bus","train","truck","boat","traffic light","fire hydrant","stop sign","parking meter","bench","bird","cat","dog","horse","sheep","cow","elephant","bear","zebra","giraffe","backpack","umbrella","handbag","tie","suitcase","frisbee","skis","snowboard","sports ball","kite","baseball bat","baseball glove","skateboard","surfboard","tennis racket","bottle","wine glass","cup","fork","knife","spoon","bowl","banana","apple","sandwich","orange","broccoli","carrot","hot dog","pizza","donut","cake","chair","couch","potted plant","bed","dining table","toilet","tv","laptop","mouse","remote","keyboard","cell phone","microwave","oven","toaster","sink","refrigerator","book","clock","vase","scissors","teddy bear","hair drier","toothbrush" };
        }

        private IEnumerator LoadModelAsync()
        {
            isLoading = true;
            string effectivePath = !string.IsNullOrEmpty(overrideOnnxPath) ? overrideOnnxPath : Path.Combine(Application.streamingAssetsPath, modelPath);
            byte[] bytes = null;
            ...
            using (var req = UnityWebRequest.Get(effectivePath))
            {
                ...
                else UnityEngine.Debug.LogError("[ONNXEngine] Model load failed: " + req.error + " | " + effectivePath);
            }
#else
            if (File.Exists(effectivePath)) bytes = File.ReadAllBytes(effectivePath);
            else UnityEngine.Debug.LogError("[ONNXEngine] Model file not found: " + effectivePath);
            yield return null;
#endif
            ...
            try
            {
                ...
                isLoaded = true;
                useDemoMode = false;
                UnityEngine.Debug.Log("[ONNXEngine] Real ONNX model loaded. Input=" + ortInputName + " Output=" + ortOutputName);
            }
            catch (Exception ex)
            {
        ...
        private DenseTensor<float> TextureToNCHWTensor(Texture2D src)
        {
            ...
            var t = new DenseTensor<float>(new[] { 1, 3, inputHeight, inputWidth });
        ...
        private List<DetectionResult> ParseYolo(Tensor<float> o)
        {
            int[] d = o.Dimensions.ToArray();
            var list = new List<DetectionResult>();
            if (d.Length != 3) return list;

            if (d[0] == 1 && d[1] >= 6 && d[2] > d[1])
                ParseAttributesAnchors(o, d[1], d[2], list);
            else if (d[0] == 1 && d[2] >= 6 && d[1] > d[2])
                ParseAnchorsAttributes(o, d[1], d[2], list);
        ...
        private void AddDetection(List<DetectionResult> list, int cls, float conf, float x0, float y0, float x1orW, float y1orH, bool xyxy)
        {
            ...
            if (xyxy)
            {
                float x1 = NormalizeCoord(x0, inputWidth), y1 = NormalizeCoord(y0, inputHeight);
                float x2 = NormalizeCoord(x1orW, inputWidth), y2 = NormalizeCoord(y1orH, inputHeight);
                r = Rect.MinMaxRect(Mathf.Min(x1,x2), Mathf.Min(y1,y2), Mathf.Max(x1,x2), Mathf.Max(y1,y2));
            }
            else
            {
                float cx = NormalizeCoord(x0, inputWidth), cy = NormalizeCoord(y0, inputHeight);
                float w = NormalizeSize(x1orW, inputWidth), h = NormalizeSize(y1orH, inputHeight);
                r = new Rect(cx - w * 0.5f, cy - h * 0.5f, w, h);
            }
            if (r.width <= 0.01f || r.height <= 0.01f || r.width > 1.2f || r.height > 1.2f) return;
            r.x = Mathf.Clamp01(r.x); r.y = Mathf.Clamp01(r.y); r.width = Mathf.Clamp01(r.width); r.height = Mathf.Clamp01(r.height);
            string label = cls >= 0 && cls < labels.Length ? labels[cls] : "class_" + cls;
            list.Add(new DetectionResult(cls, label, conf, r));
        }

        private float NormalizeCoord(float v, int size) => Mathf.Abs(v) > 1.5f ? v / size : v;
        private float NormalizeSize(float v, int size) => Mathf.Abs(v) > 1.5f ? v / size : v;
        private float SigmoidIfNeeded(float v) => (v < 0f || v > 1f) ? 1f / (1f + Mathf.Exp(-v)) : v;
        ...
        private List<DetectionResult> ApplyNMS(List<DetectionResult> dets)
        {
            ...
            foreach (var d in dets)
            {
                ...
                {
                    if (d.classId == k.classId && ComputeIOU(d.boundingBox, k.boundingBox) > nmsThreshold)
                    { keep = false; break; }
                }
                if (keep) kept.Add(d);
            }
